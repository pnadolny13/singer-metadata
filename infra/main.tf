...
resource "aws_apigatewayv2_integration" "singer_metadata" {
  api_id = aws_apigatewayv2_api.lambda.id

  integration_uri    = aws_lambda_function.lambda_singer_metadata.invoke_arn
...
resource "aws_apigatewayv2_route" "tap" {
  ...
  route_key = "GET /v1"
  target    = "integrations/${aws_apigatewayv2_integration.singer_metadata.id}"
}
